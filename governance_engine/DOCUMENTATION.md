# Governance engine — full documentation

This document describes everything under `governance_engine/`: concepts, data model, ingestion, the SQLite database, CLI commands, the web UI, and the detector system.
...
`python -m governance_engine db exchanges` means: **list rows from the `exchanges` table** — i.e. list **stored exchanges**, not raw JSONL lines.
...
```bash
python3 -m governance_engine ingest --project-id <PROJECT_ID> [--chat-id <CHAT_ID>] [--db-path PATH] [--dry-run]
```
...
```bash
python3 -m governance_engine db <subcommand> [--db-path PATH] ...
```
...
python3 -m governance_engine serve [--db-path PATH] [--host 127.0.0.1] [--port 5050]
```
...
- **CLI:** `python3 -m governance_engine db detect --score [--alert-threshold 75]` adds a **`policy`** object to JSON output and prints score / raw / combos in text mode.
...
| Command | Effect |
|---------|--------|
| `python3 -m governance_engine ingest ...` | Ingest (see §7). |
| `python3 -m governance_engine db ...` | DB CLI (see §8). |
| `python3 -m governance_engine serve ...` | Web UI (see §9). |
| `python3 -m governance_engine` | Prints usage if no subcommand. |

Equivalent direct modules:

- `python3 -m governance_engine.ingest`
- `python3 -m governance_engine.cli`
- `python3 -m governance_engine.web`
...
The sibling folder **`ide-interceptor/`** (repo root) is a **separate** tool: it **tails** Cursor transcript JSONL files in real time and writes **`captured_chat.jsonl`**. It is **not** required for `governance_engine` ingest, which reads the same transcripts **directly** from `~/.cursor/projects/...`.
