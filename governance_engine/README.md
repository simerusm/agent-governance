...
Reads Cursor `agent-transcripts/<chat_id>/<chat_id>.jsonl` files, parses **user / assistant** turns into normalized rows, and stores them in **SQLite** for governance scanning and analysis.

Python package: **`governance_engine`** (run as `python3 -m governance_engine …`).
...
python3 -m governance_engine ingest --project-id Users-mahesh-Code-quick-mvp
```
...
```bash
python3 -m governance_engine db stats
python3 -m governance_engine db chats
python3 -m governance_engine db exchanges --project-id Users-mahesh-Code-quick-mvp --limit 20
python3 -m governance_engine db exchanges --json --limit 5
```
...
python3 -m governance_engine db detect --limit 50

# Only user messages (common for “what did the user paste?”)
python3 -m governance_engine db detect --text-source user --hits-only

# One chat, full JSON (redacted snippets only)
python3 -m governance_engine db detect \
  --project-id Users-mahesh-Code-quick-mvp \
  --chat-id b608d697-fc98-423a-8ffd-5f3409aacbbd \
  --json

# Short summary only (no per-finding reason/snippet lines)
python3 -m governance_engine db detect --compact --limit 20

# Policy score (0–100) + alert if score >= threshold (default 75)
python3 -m governance_engine db detect --score --hits-only --limit 10
python3 -m governance_engine db detect --score --alert-threshold 80 --json --limit 5
```

Weights and combo rules: `governance_engine/scoring/registry.py`.

## Web UI

```bash
python3 -m governance_engine serve --port 5050
```
